...
- 如果服务未运行，自动通过 `node cli.js start --daemon` 在后台启动
...
4. **服务守护** (`hooks/server-guard.js`) -- 钩子自动拉起服务的核心模块。通过 TCP 端口检测 + PID 文件双重验证服务状态。使用 `node cli.js start --daemon` 启动服务（跨平台统一方案）。
